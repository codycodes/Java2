...
public class Grid {
	...
	/**
	 * Checks to see if a row of squares needs to be removed
	 */
	public void checkRows() {
		// TODO: Finish this method
		// Iterate through all of the squares, see if "row" is full
		// if row is full, clear, then move everything down to that row.
		// then set the above row to empty
		for (int i = 0; i < HEIGHT; i++) {
			int count = 0; // used to count the number of set squares
			// loop through every square in a row
			for (int j = 0; j < WIDTH; j++) {
				// if a row is completely full, then clear it
				if (this.isSet(i, j)) {
					count++;
					if (count == WIDTH) {
						clearRow(i);
					...
					break; // break out of checking each col in the row
				}
			}
		}
	}
	...
	/**
	 * removes all solid rows of squares.
	 * 
	...
	public void clearRow(int row) {
		...
		for (int r = row - 1 ; r >= 0; r--) {
			for (int col = 0; col < WIDTH; col++) {
				...
				board[r + 1][col].setColor(board[r][col].getColor());
			}
		}
